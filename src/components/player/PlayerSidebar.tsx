...
export default function PlayerSidebar({ player }: PlayerSidebarProps) {
  ...
  return (
    <div className="w-80 flex flex-col gap-4 self-start relative">
        ...
        {(player.player_rating || player.player_ranking) && (
          <div className="w-full grid grid-cols-3 gap-2 text-center pt-2">
            ...
            <div className="flex flex-col items-center">
              <span className="text-xs text-[#6d6d6d] uppercase">Level</span>
      ...
      {/* Stats Container (Market Value & Community Rating) */}
                ...
                >
                  ...
                  <g mask={`url(#mask_rating_${i})`}>
                    <path
                      ...
                      fill={i < Math.round(player.average_report_rating || 0) ? "#8c1a10" : "#e5e7eb"}
                    />
                  </g>
                </svg>
             ))}
          </div>
          <p className="text-xs text-[#6d6d6d]">Community Rating</p>
          {player.average_report_rating && player.average_report_rating > 0 && (
             <span className="text-sm font-bold text-[#8c1a10] mt-1">{player.average_report_rating.toFixed(1)}</span>
          )}
        </div>
      </div>
    </div>
